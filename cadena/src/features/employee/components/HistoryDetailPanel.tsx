...
export default function HistoryDetailPanel({ activity, user, onClose }: HistoryDetailPanelProps) {
  ...
  const renderChangeValue = (field: string, value: string, isNew: boolean) => {
    if (field === "profile_photo") {
      return (
        ...
        >
          {value ? (
            <img alt="Photo" className="w-full h-full object-cover" src={`${ENV.API_BASE_URL}${value}`} />
          ) : (
    ...
    if (field === "skills") {
      ...
      return (
        <div
          className={`p-3 rounded-lg text-xs flex flex-wrap gap-1 ${
            isNew
        ...
        <div className="space-y-10 flex-1">
          ...
          <div className="bg-surface p-6 rounded-xl space-y-4">
            <div className="flex justify-between">
              <span className="text-xs font-semibold text-on-surface-variant">Activity ID</span>
              <span className="text-xs font-mono text-on-surface truncate ml-4">
                {activity.id.split("-")[0]?.toUpperCase()}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-xs font-semibold text-on-surface-variant">Device</span>
              <span
                className="text-xs text-on-surface flex items-center gap-1 max-w-[150px] truncate"
                title={activity.device || ""}
              >
                <span className="material-symbols-outlined text-sm">{getDeviceIcon(activity.device || "")}</span>
          ...
          <div className="space-y-6">
            <h4 className="text-sm font-black uppercase tracking-widest text-on-surface-variant">Field Comparison</h4>
            ...
            {activity.changes.map((change, idx) => (
              <div key={`${change.field}-${idx}`} className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-on-surface uppercase tracking-wider">{change.field}</span>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    {change.field === "profile_photo" && (
                      <p className="text-[10px] font-bold text-primary uppercase w-full truncate">New Value</p>
                    )}
